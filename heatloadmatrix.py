...
class HMainWindow(QtGui.QWidget):
    ...
    def run_begin(self):
        ...
        # prog mesh
        if self.ui.mesh_progressive.isChecked(): 
            run["mesh"]="progressive"
            run["mesh_level"]=self.ui.mesh_level.text()
        else:
            run["mesh"]="uniform"
    ...
    def main_load_values(self):
        ...
        # mesh
        if run["mesh"]=="uniform": self.ui.mesh_uniform.setChecked(True)
        else:
            self.ui.mesh_progressive.setChecked(False)
            self.ui.mesh_level.setText(str(run["mesh_level"]))
