...
class Back(rectangle_grid.pc):
    ...
    def back_values(self,source):
        ...
        self.s_flux_test=False #TURN OFF!
        ...
        #for some reason, hd, and vd get reverse inside the code. This will fix that error, in a highly rudimentary way.
        #self.vd,self.hd=self.hd, self.vd
    ...
    def generate_energy_axis(self):
        """returns energy values, 500-100,000 eV with 5000 divisions, locked."""
        """
        if self.source=="wig":
        ...
        #Do once, then redefine
        f=open("pickle\\energy_axis.pkl","wb")
        pickle.dump(sdata,f)
        f.close()
        #sdata=pickle.load(open("mu_data//Xx.pkl","rb"))   
    ...
    def integrated_source_power(self,s_flux):
        ...
        print((len(s_flux),len(areas)), (len(s_flux[0]),len(areas[0])), (len(s_flux[0][0]), len(ea)))
        for i in range(0,len(s_flux)):
            for j in range(0,len(s_flux[0])):
    ...
    def source_flux(self):
        ...
        if self.s_flux_test==True:
            f=open("pickle\\raw_flux.pkl","rb")
            s_flux=pickle.load(f)
            f.close()
        else:            
            pc=self.rect_centers()
            ea=self.generate_energy_axis()
            dE=ea[1]-ea[0]
            #print(self.vd+2,self.hd+2,len(pc),len(pc[0]))
            if self.LIP==True:
                jlimit=self.vd+2
                ilimit=self.hd+2
            else:
                jlimit=self.vd
                ilimit=self.hd
            #print(depth(pc),[ilimit,len(pc)],[len(pc[0]),jlimit])
            #assert ilimit==len(pc)
            #assert jlimit==len(pc[0])
            
            s_flux=cell2(jlimit,ilimit)
            uncorrected_flux=cell2(jlimit,ilimit)
            ...
            print((len(s_flux),ilimit), (len(s_flux[0]),jlimit))     
            for i in range(0,ilimit):
                for j in range(0,jlimit):
                    x_offset=pc[i][j][0]
                    y_offset=pc[i][j][1]
            ...
            f=open("pickle\\raw_flux.pkl","wb")
            pickle.dump(s_flux,f)
            f.close()
    ...
    def write_slice_to_table(self,voxel_absorbed_power_density,title):
        ...
        for k in range(0,zlen):
            ...
            if (len(power_temp) < len(vr)):
                print('len(power_temp) >= i layer', k+1, len(power_temp), i, len(vr))
