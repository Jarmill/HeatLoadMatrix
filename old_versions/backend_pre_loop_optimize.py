        ...
        for k in range(0,klimit):
            for i in range(0,ilimit):
                for j in range(0,jlimit):
                    ...
                    slice_volumes[k][i][j]=areas[j][i]*self.thickness[k]*10*(10**-9)        
        ...
        for i in range(0,len(s_flux)):
            for j in range(0,len(s_flux[0])):
                for m in range(0, len(ea)):
                    intpow+=(s_flux[i][j][m]*ea[m]*1.60217646E-19)*(areas[j][i]/full_area)#the dA element of the integral
        ...
        for k in range(0,klimit):
            for i in range(0,ilimit):
                for j in range(0,jlimit):
                    ...
                    total_integrated_power+=v_power[k][i][j]*(areas[j][i]/full_area) ### *self.thickness[k]/sumthick ### TESTING FIX
