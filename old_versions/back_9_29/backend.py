...
class Back(rectangle_grid.pc):
    ...
    def write_slice_to_table(self,voxel_absorbed_power_density,title):
        ...
        for k in range(0,zlen):
            ...
            power_temp=([cell1(xlen)]+power_temp+[cell1(xlen)])
            ##print(power_temp)
            #vertical zero padding
            print(power_temp)
            for i in range(0,ylen+2):
                power_temp[i]=([hr[i]]+[0]+power_temp[i]+[0])
                                         
            #for i in range(0,len(vr+2)):
            #    power_temp[i]=[vr[i]]+power_temp[i]
            
            vr.insert(0,10*self.thickness[k])
            power_temp.insert(0,vr)
            power_temp_s=[list(map(str,i)) for i in power_temp]
